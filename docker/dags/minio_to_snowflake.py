...
def load_to_snowflake(**kwargs) :
    local_files = kwargs["ti"].xcom_pull(task_ids="minio_to_snowflake_banking")
    if not local_files:
...
with DAG(
      ...
      description="Load data from MinIO parquet into Snowflake RAW tabless",
      schedule='*/1 * * * *',
      start_date=datetime(2025, 1, 1),
      catchup=False,
)as dag: 
     task1 = PythonOperator(
           task_id = "minio_to_snowflake_banking",
           python_callable=download_from_minio,
     )
     task2 = PythonOperator(
             task_id = "load_to_snowflake_banking",
